...
def show() -> None:
    ...
    st.markdown("Analiza qué **rutas**, **productos** y **zonas** generan más ganancia.")
    ...
    if summary["num_sales"] == 0:
        st.info("No hay ventas en este período. Ve a **💰 Ventas** para registrar transacciones.")
        return
...
def _by_route(db: Database, uid: int, days: int) -> None:
    ...
    with c1:
        fig = px.bar(
            ...
            x="route", y=["revenue", "cost", "profit"], barmode="group",
            title="Ingresos, costo y ganancia por ruta",
            labels={"value": "$", "variable": "Métrica", "route": "Ruta"},
            color_discrete_map={"revenue": "#667eea", "cost": "#f56565", "profit": "#48bb78"},
        )
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        fig2 = px.bar(
            df.sort_values("margin_pct"),
            x="margin_pct", y="route", orientation="h",
    ...
    fig3 = px.pie(df, names="route", values="total_units",
                  title="Distribución de unidades vendidas por ruta", hole=0.4)
...
def _by_product(db: Database, uid: int, days: int) -> None:
    ...
    with c2:
        if len(df) > 1:
            ...
            if worst["margin_pct"] < 10:
                st.warning(f"Menor margen: **{worst['product_name']}** — {worst['margin_pct']:.1f}%")

    if not df.empty:
        fig_tree = px.treemap(
            df, path=["category", "product_name"], values="revenue",
            color="margin_pct", color_continuous_scale="RdYlGn",
    ...
    with c2:
        fig_sc = px.scatter(
            df, x="total_units", y="profit", size="revenue",
            color="margin_pct", hover_name="product_name",
            color_continuous_scale="RdYlGn",
            title="Unidades vendidas vs Ganancia",
            labels={"total_units": "Unidades", "profit": "Ganancia ($)", "margin_pct": "Margen (%)"},
        )
        st.plotly_chart(fig_sc, use_container_width=True)

    df_show = df[["sku", "product_name", "category", "num_sales",
                  "total_units", "revenue", "cost", "profit", "margin_pct"]].copy()
    df_show.columns = ["SKU", "Producto", "Categoría", "Ventas",
...
def _movements(db: Database, uid: int) -> None:
    ...
    st.caption("Cada venta actualiza el inventario automáticamente. Aquí puedes auditar todos los movimientos.")

    col1, _ = st.columns([1, 3])
    with col1:
        days = st.selectbox("Período", [7, 15, 30, 60, 90], index=2,
                            format_func=lambda x: f"Últimos {x} días", key="mov_days")
    ...
    by_day = df.groupby(["date", "movement_type"])["quantity"].sum().reset_index()
    if not by_day.empty:
        by_day["movement_type"] = by_day["movement_type"].map(
            {"sale": "Ventas", "purchase": "Compras", "adjustment": "Ajustes"}
        ).fillna(by_day["movement_type"])
        fig = px.bar(
            by_day, x="date", y="quantity", color="movement_type",
            title="Movimientos de stock por día",
            labels={"quantity": "Unidades", "date": "Fecha", "movement_type": "Tipo"},
            color_discrete_map={"Ventas": "#f56565", "Compras": "#48bb78", "Ajustes": "#ecc94b"},
        )
        st.plotly_chart(fig, use_container_width=True)
