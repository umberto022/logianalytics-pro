...
def _profile(user: dict) -> None:
    ...
    with st.form("profile_form"):
        ...
        with col_b:
            change_pass  = st.form_submit_button("🔐 Cambiar contraseña", use_container_width=True)
...
def _company(user: dict) -> None:
    ...
    if not company:
        ...
        with st.form("new_company_form"):
            ...
            submit = st.form_submit_button("Registrar empresa", type="primary", use_container_width=True)
        if submit:
            if not name:
    ...
    with st.form("edit_company_form"):
        c1, c2 = st.columns(2)
        with c1:
            name     = st.text_input("Nombre *", value=company["name"])
