...
def show() -> None:
    ...
    tab_list, tab_add, tab_import = st.tabs([
        "📋 Mis productos", "➕ Agregar producto", "📤 Importar / Exportar",
    ])
...
def _product_list(db: Database, uid: int, items: list) -> None:
    ...
    if not alerts.empty:
        with st.expander(f"⚠️ {len(alerts)} producto(s) con stock bajo", expanded=n_crit > 0):
            for _, r in alerts.iterrows():
                ...
                st.markdown(
                    f'<div class="{cls}"><b>{r["name"]}</b>'
                    f'{" · " + r["color"] if r.get("color") else ""}'
                    f' — stock: <b>{int(r["current_stock"])}</b>'
                    f' / mín: {int(r["min_stock"])} / máx: {int(r["max_stock"])}</div>',
    ...
    labels = {
        f"{r['name']}  ({r['category']} · {r['color'] or 'sin color'})  — stock {r['current_stock']}": r
        for r in items
    }
    ...
    if not dff.empty:
        ...
        fig = px.bar(
            ...
            x="name", y="current_stock", color="estado",
...
def _add_form(db: Database, uid: int, company_id) -> None:
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        with c1:
            ...
            tipo  = st.text_input("Tipo *",     placeholder="ej. Ropa, Electrónico")
        ...
        submit = st.form_submit_button("➕ Agregar al inventario", type="primary", use_container_width=True)
...
def _import_export(db: Database, uid: int, items: list) -> None:
    ...
    with col_exp:
        ...
        if items:
            ...
            st.download_button("⬇️ Descargar inventario (CSV)", data=csv,
    ...
    with col_imp:
        ...
        st.caption("Columnas **obligatorias**: `producto`, `tipo`, `stock`, `stock_minimo`, `stock_maximo`")
        ...
        st.download_button("⬇️ Descargar plantilla", data=template.encode("utf-8"),
                           file_name="plantilla_inventario.csv", mime="text/csv", use_container_width=True)
        ...
        if up is not None:
            try:
                ...
                if miss:
                    ...
                    if st.button("✅ Importar al inventario", type="primary", use_container_width=True):
                        ok_c, bad = 0, []
