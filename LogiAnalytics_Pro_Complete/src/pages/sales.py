...
def _register_sale(db: Database, uid: int, company_id) -> None:
    ...
    if submit:
        if quantity > selected["current_stock"]:
            st.error(
                f"Stock insuficiente. Disponible: **{selected['current_stock']}** unidades."
            )
        else:
...
def _sales_history(db: Database, uid: int) -> None:
    ...
    show_cols = [
        "sale_date", "sku", "product_name", "quantity",
    ...
    st.dataframe(
        df[show_cols].rename(columns=renamed),
        use_container_width=True,
        hide_index=True,
    )
    ...
    st.download_button(
        "⬇️ Descargar CSV",
        data=csv,
        file_name=f"ventas_{days}d.csv",
        mime="text/csv",
    )
