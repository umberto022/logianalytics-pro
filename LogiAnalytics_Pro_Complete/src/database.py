...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logi.db")
...
class Database:
    ...
    def _init_db(self):
        ...
        # Migrate old schema if needed (users table without email column)
        c.execute("PRAGMA table_info(users)")
        existing_cols = [row[1] for row in c.fetchall()]
        if existing_cols and "email" not in existing_cols:
            for tbl in ["sessions", "inventory_movements", "sales", "inventory", "users"]:
    ...
    def update_user(self, user_id: int, email=None, full_name=None, company_name=None,
                    phone=None, password=None, company_id=None) -> bool:
        sets, vals = [], []
        ...
        if sets:
            ...
            conn.commit()
            conn.close()
        # If company_name provided and user has a company, update it
        if company_name:
    ...
    def _user_dict(row) -> dict:
        cols = ["id","username","email","full_name","password_hash","phone","role",
                "is_active","subscription_plan","company_id","created_at","last_login"]
        d = dict(zip(cols, row))
    ...
    def get_company(self, company_id: int) -> Optional[dict]:
        ...
        if not row:
            return None
        cols = ["id","name","rif","address","phone","email","industry","country","owner_id","created_at"]
    ...
    def get_inventory_movements(self, user_id: int, days: int = 30) -> list:
        ...
        rows = c.fetchall()
        conn.close()
        cols = ["id","inventory_id","movement_type","quantity","reference","note",
                "created_at","sku","name"]
        return [dict(zip(cols, r)) for r in rows]
    ...
    def get_sales(self, user_id: int, days: int = 30) -> list:
        ...
        rows = c.fetchall()
        conn.close()
        cols = ["id","user_id","company_id","inventory_id","sku","product_name","category",
                "quantity","unit_price","unit_cost","route","zone","client",
                "sale_date","total_revenue","total_cost","profit"]
        return [dict(zip(cols, r)) for r in rows]
